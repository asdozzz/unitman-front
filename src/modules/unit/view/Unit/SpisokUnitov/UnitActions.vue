...
<template>
  <template v-if="item.unitSozdanSystemoi || userId === item.authorId || getUserRoleByProjectIdAndUserId(item.projectId, userId as string) === 'ADMIN'">
    ...
    <q-btn size="md" square color="black" padding="5px 6px" v-if="esliJdemRunner(item)" icon="refresh" @click="obnovit(item.id)" :loading="getUnitLoader(item.id)">
    ...
    <template v-for="command in item.commands">
      ...
      <q-btn v-if="command === 'nachatObnovleniePosleZapuska'" size="sm" square color="black" padding="5px 6px" icon="update" @click="obnovitKodMoegoUnita(item.id)" :loading="getUnitLoader(item.id)">
        <q-tooltip>{{$t('unit.spisok_unitov.card.buttons.fetch_changes')}}</q-tooltip>
      </q-btn>
      ...
      <q-btn v-if="command === 'proveritKonteinerUnita' && isAdmin" size="sm" square color="black" padding="5px 6px" icon="sync" @click="proveritKonteinerUnita(item.id)" :loading="getUnitLoader(item.id)">
