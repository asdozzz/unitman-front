...
<template>
  <div class="flex flex-center q-mt-lg">
    <q-card square class="shadow-2 my_card" bordered v-if="!tokenData">
      <q-card-section class="text-center">
        <div class="text-grey-9 text-h5 text-weight-bold">{{$t('account.login_form.caption')}}</div>
      </q-card-section>
      <q-card-section>
        <q-input square name="login" v-model="loginView.form.login" dense outlined :label="$t('account.login_form.email_label')"></q-input>
        <q-input square name="pass" v-model="loginView.form.pass" dense outlined class="q-mt-md" type="password" :label="$t('account.login_form.password_label')"></q-input>
      </q-card-section>
      <q-card-section>
        <q-btn square color="dark" rounded size="md" @click="onSubmit" :label="$t('account.login_form.button_login')" no-caps class="full-width"></q-btn>
      </q-card-section>
    </q-card>
